    ...
    @endif
        <div @if($label) class="mt-2" @endif>
            <div class="flex items-center rounded-md bg-white pl-3 outline-1 -outline-offset-1 outline-gray-400/75 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-indigo-600">
                <input type="{{ !$type ? 'text' : $type }}" id="{{ $name }}" @if($type != "password") value="{{ old($name) }}" @endif {{$attributes->merge(['class' => "block min-w-0 grow py-1.5 pr-3 pl-1 text-base text-gray-900 placeholder:text-gray-500 focus:outline-none sm:text-sm/6"])}}>
            </div>
            @if(!$noErrors)
                <x-input-error :$name/>
            @endif
