...
<form {{ $attributes->merge(['class' => "m-auto w-fit border border-gray-500/45 rounded-2xl px-7 py-11 bg-gray-100/60 my-13"]) }}>
    ...
    <div class="mt-6 flex items-center justify-center gap-x-6 text-shadow-lg/12.5">
        ...
        <button id="accountFormSubmission" @if(request()->is("/register")) disabled @endif type="submit" class="cursor-pointer rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600">Kontynuuj</button>
    </div>
</form>
