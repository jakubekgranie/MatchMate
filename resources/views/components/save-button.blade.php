@props(["svgIconId", "text" => "Zatwierdź", "fill" => "oklch(70.7% 0.022 261.325)"])
<button  {{ $attributes->merge(["class" => "select-none rounded-md text-sm font-semibold shadow-xs bg-gray-800 text-gray-300 hover:bg-gray-700 cursor-pointer focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gray-400 duration-100 font-system flex items-center justify-center"]) }}>
    <svg id="{{ $svgIconId }}" class="w-4 h-4 mr-2 duration-100" viewBox="0 0 495 495" style="fill: {{ $fill }}; shape-rendering:geometricPrecision; text-rendering:geometricPrecision; image-rendering:optimizeQuality; fill-rule:evenodd; clip-rule:evenodd">
        @if($slot->isEmpty())
