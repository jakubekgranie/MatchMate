<x-layout page-title="Profil" :flex-center="true" class="mb-23">
    <div class="outline-lime-400 hidden"></div> <!-- A storage element -->
    <x-slot:scripts>
    ...
    <div class="border-2 border-gray-500/65 bg-white/30 rounded-2xl flex flex-col gap-7 py-7 px-9.5 shadow-sm shadow-gray-300 my-13">
        ...
        <div class="flex justify-between items-center">
            <h1 class="text-2xl text-gray-950 font-bold">Personalia</h1>
            <div class="flex items-center">
                <div class="h-63 w-0.5 bg-stone-400 mx-8 rounded-sm"></div>
                <div class="m-auto">
                    <x-update-form-container>
                        <form method="POST" class="grid grid-cols-2 gap-3.5" action="/profile/email">
                            @csrf
                            @method('PATCH')
                            <x-form-input type="email" name="email" maxlength="254" :label="false" placeholder="Adres e-mail" :override-grid="true"/>
                            <x-save-button id="changeMail" type="submit" class="h-full w-full" fill="oklch(87.2% 0.01 258.338)" text="Zmień e-mail">
                    ...
                    <x-update-form-container class="mt-7">
                        <form method="POST" class="grid grid-cols-2 gap-3.5" action="/profile/password">
                            @csrf
                            @method('PATCH')
                            <x-form-input type="password" name="password" maxlength="64" :label="false" placeholder="Hasło" :override-grid="true"/>
                            <x-form-input type="password" name="password_confirmation" maxlength="64" :label="false" placeholder="Powtórz hasło" :override-grid="true"/>
                    ...
                    <x-update-form-container class="mt-7">
                        <form method="POST" class="h-9" action="/profile/delete">
                            @csrf
                            @method('DELETE')
                            <x-save-button id="imageReset" type="submit" class="bg-red-700 hover:bg-red-600 text-gray-100 h-full w-full" fill="oklch(87.2% 0.01 258.338)" text="Usuń konto">
